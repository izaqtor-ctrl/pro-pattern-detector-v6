...
st.set_page_config(
    page_title="Pro Pattern Detector v6.0", 
    layout="wide"
)
...
def create_chart(data, ticker, pattern_type, pattern_info, levels, market_context):
    """Create enhanced chart with volume analysis and timing context"""
    fig = make_subplots(
        rows=3, cols=1,
        subplot_titles=(
            f'{ticker} - {pattern_type} | {levels["target_method"]} | {market_context["day"]}',
            'MACD Analysis', 
...
def main():
    st.title("🎯 Pro Pattern Detector v6.0")
    st.markdown("**Market Timing & Enhanced Volume Analysis** - Professional Pattern Recognition with Timing Intelligence")
    ...
    with st.expander("🆕 What's New in v6.0 - Market Timing Intelligence"):
        st.markdown("""
        ### 🕐 **Professional Market Timing**
        
        **📅 Day-of-Week Analysis**:
        - **Weekend**: Pattern review mode, gap risk warnings
        - **Monday**: Gap validation required, pattern confirmation needed
        - **Mid-Week** (Tue-Thu): Optimal entry timing
        - **Friday**: High volume required for weekend holds
        
        **⚠️ Smart Confidence Adjustments**:
        - **Weekend Analysis**: -5% confidence penalty
        - **Friday Entry**: -15% without exceptional volume (2.0x+)
        - **Monday Gap Risk**: Validation reminders
        - **Mid-Week Bonus**: +2% confidence for optimal timing
        
        **🎯 Entry Timing Guidance**:
        - **Real-time recommendations** based on current day/time
        - **Gap risk assessment** (High/Medium/Low)
        - **Volume requirements** adjusted by day
        - **Pattern validity** context for each situation
        
        **📊 Enhanced Visuals**:
        - Market timing status in chart titles
        - Entry timing annotations on price charts
        - Gap risk warnings prominently displayed
        
        **This matches how professional traders time their entries!** ⏰
        """)
        ...
        if tickers and selected_patterns:
                    ...
                    for pattern in selected_patterns:
                        ...
                        if require_volume:
                            ...
                            if volume_multiplier < required_threshold:
                                skip_pattern = True
                                st.info(f"⏭️ {pattern}: {confidence:.0f}% - Filtered by volume requirement")
                                continue
                        ...
                        if detected and confidence >= min_confidence:
                            ...
                            fig = create_chart(data, ticker, pattern, info, levels, market_context)
            ...
            if results:
                ...
                st.download_button(
                    "📥 Download Results",
                    csv,
                    f"patterns_v6_{datetime.now().strftime('%Y%m%d_%H%M')}.csv",
                    "text/csv"
                )
            else:
